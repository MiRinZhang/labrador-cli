...
module.exports = function* buildXML(from, to) {
  ...
  if(process.env.APP_MODEL === 'alipay') {
    // 替换判断条件
    xml = xml.replace(/wx:/g, 'a:');
    xml = xml.replace(/xmlns:wx=""/, '');
    // 替换bind事件
    xml = xml.replace(/bind([A-Za-z]+)/g, ($0, $1) => `on${$1[0].toUpperCase()}${$1.slice(1)}`);
    // 替换catch事件
    xml = xml.replace(/catch([A-Za-z]+)/g, ($0, $1) => `catch{$1[0].toUpperCase()}${$1.slice(1)}`);
  }
