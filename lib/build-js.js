...
module.exports = function* buildJS(from, to, targets, metadata) {
  ...
  // 支付宝小程序进行替换
  if(process.env.APP_MODEL === 'alipay') {
    // 替换request
    code = code.replace(/wx.request/g, 'my.httpRequest').replace(/\$applet.request/g, '$applet.httpRequest').replace(/wx,/g, 'my,').replace(/wx\./g, 'my.');
    // 替换header及请求结果status code
    code = code.replace(/header:\s/g, 'headers: ').replace('.statusCode', '.status');
  }
